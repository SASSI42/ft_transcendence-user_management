...
exports.gameInvitesRoutes = gameInvitesRoutes;
const GameInvitesService_1 = require("../services/GameInvitesService");
async function gameInvitesRoutes(fastify) {
    ...
    fastify.get('/game-invites/pending/:userId', async (request, reply) => {
        try {
            ...
            const invites = GameInvitesService_1.GameInvitesService.getPendingInvites(fastify.db, userId);
    ...
    fastify.get('/game-invites/sent/:userId', async (request, reply) => {
        try {
            const { userId } = request.params;
            const invites = GameInvitesService_1.GameInvitesService.getSentInvites(fastify.db, userId);
            return reply.code(200).send(invites);
        }
        catch (error) {
            return reply.code(400).send({ error: error.message });
