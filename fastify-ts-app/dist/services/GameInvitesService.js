...
class GameInvitesService {
    static sendGameInvite(db, senderId, receiverId, gameId) {
        ...
        if (existing)
            throw new Error('A pending game invite already exists');
        ...
        try {
            ...
            inviteMessage = MessagesService_1.MessagesService.sendMessage(db, senderId, receiverId, `INVITE::${id}`, true);
        }
        catch (err) {
            console.error("Failed to create invite message", err);
        }
        // Fetch row to get the exact string SQLite stored
        const row = db.prepare('SELECT * FROM game_invites WHERE id = ?').get(id);
        return {
    ...
    static acceptGameInvite(db, inviteId, userId) {
        const invite = db.prepare(`SELECT * FROM game_invites WHERE id = ? AND receiver_id = ? AND status = 'pending'`).get(inviteId, userId);
        if (!invite)
            throw new Error('Game invite not found or expired');
    ...
    static declineGameInvite(db, inviteId, userId) {
    ...
    static cancelGameInvite(db, inviteId, senderId) {
        const invite = db.prepare(`SELECT * FROM game_invites WHERE id = ? AND sender_id = ? AND status = 'pending'`).get(inviteId, senderId);
        if (!invite)
            throw new Error('Game invite not found');
    ...
    static cancelInvitesFromSender(db, senderId) {
        const invites = db.prepare(`SELECT * FROM game_invites WHERE sender_id = ? AND status = 'pending'`).all(senderId);
        if (invites.length > 0) {
    ...
    static cancelInvitesBetween(db, userId1, userId2) {
    ...
    static getPendingInvites(db, userId) {
        ...
        `);
        return stmt.all(userId, userId).map((row) => this.mapRow(row));
    }
    static getSentInvites(db, userId) {
        const stmt = db.prepare(`
            ...
            ORDER BY gi.created_at DESC
        `);
        return stmt.all(userId).map((row) => this.mapRow(row));
    }
    static getInviteById(db, inviteId) {
        const row = db.prepare(`SELECT * FROM game_invites WHERE id = ?`).get(inviteId);
        if (!row)
            return undefined;
        return this.mapRow(row);
    }
    static expireInvite(db, inviteId) {
        db.prepare(`UPDATE game_invites SET status = 'expired' WHERE id = ?`).run(inviteId);
        return { success: true };
    }
    static cleanupExpiredInvites(db) {
    ...
    static getInviteHistory(db, userId, limit = 50) {
        const stmt = db.prepare(`
            SELECT gi.*, 
        ...
        return stmt.all(userId, userId, userId, limit).map((row) => this.mapRow(row));
    }
...
exports.GameInvitesService = GameInvitesService;
