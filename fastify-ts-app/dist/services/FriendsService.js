...
class FriendsService {
    // Send friend request
    static sendFriendRequest(db, userId, friendId) {
        ...
        `).get(friendId, userId); // Note: user_id=friendId (Sender), friend_id=userId (Receiver)
        if (incomingRequest) {
    ...
    static acceptFriendRequest(db, friendshipId) {
        const stmt = db.prepare(`
            UPDATE friendships 
            SET status = 'accepted'
    ...
    static declineFriendRequest(db, friendshipId) {
        // 1. Fetch first so we know who to notify
        const request = db.prepare('SELECT * FROM friendships WHERE id = ?').get(friendshipId);
        if (!request) {
    ...
    // Remove friend
    static removeFriend(db, userId, friendId) {
    ...
    // Block user
    static blockUser(db, userId, blockedUserId) {
    ...
    // Unblock user
    static unblockUser(db, userId, blockedUserId) {
    ...
    static getFriends(db, userId) {
        ...
        return stmt.all(userId, userId, userId, userId).map((friend) => ({
            id: friend.id,
    ...
    static getFriendsToNotify(db, userId) {
        const stmt = db.prepare(`
            SELECT 
                CASE 
                    WHEN f.user_id = ? THEN f.friend_id 
    ...
    // Get blocked users
    static getBlockedUsers(db, userId) {
        const stmt = db.prepare(`
    ...
    static getPendingRequests(db, userId) {
        const stmt = db.prepare(`
            SELECT 
                u.id, 
                u.username, 
    ...
    static getSentRequests(db, userId) {
        const stmt = db.prepare(`
            SELECT 
                f.id,
                f.user_id,
    ...
    static areFriends(db, userId1, userId2) {
        const stmt = db.prepare(`
    ...
    // Helper: Is blocked?
    static isBlocked(db, userId, potentialBlockerId) {
        // "Is userId blocked by potentialBlockerId?"
        const stmt = db.prepare(`
            SELECT count(*) as count FROM blocked_users 
            WHERE blocker_id = ? AND blocked_id = ?
        `);
        const result = stmt.get(potentialBlockerId, userId);
        return result.count > 0;
    }
}
