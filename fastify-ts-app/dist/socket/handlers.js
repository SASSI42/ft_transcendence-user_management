...
const GameInvitesService_1 = require("../services/GameInvitesService");
...
const matchmakingService = new matchmaking_1.MatchmakingService();
const gameRoomManager = new gameRoom_1.GameRoomManager();
function setupSocketHandlers(db, io) {
    io.on('connection', (rawSocket) => {
        ...
        const updateInviteStatus = (inviteId, status) => {
            const updatedMsg = MessagesService_1.MessagesService.updateInviteMessage(db, inviteId, status);
            if (updatedMsg) {
                ...
                if (senderSocket)
                    io.to(senderSocket).emit('message_updated', updatedMsg);
                // If I am the receiver (and not blocked ideally, but the UI handles hiding), send it
                if (receiverSocket)
                    io.to(receiverSocket).emit('message_updated', updatedMsg);
            }
        };
        ...
        socket.on('send_message', async (data) => {
            try {
                if (!userId)
                    return;
                const message = MessagesService_1.MessagesService.sendMessage(db, userId, data.receiverId, data.content);
        ...
        socket.on('mark_all_read', async (data) => {
            try {
                if (!userId)
                    return;
                const result = MessagesService_1.MessagesService.markAllAsRead(db, userId, data.senderId);
        ...
        socket.on('send_game_invite', async (data) => {
            try {
                ...
                const { invite, message, isBlocked } = GameInvitesService_1.GameInvitesService.sendGameInvite(db, userId, data.receiverId, data.gameId);
                ...
                if (!isBlocked) {
                    ...
                    setTimeout(() => {
                        const currentInvite = GameInvitesService_1.GameInvitesService.getInviteById(db, invite.id);
                        if (currentInvite && currentInvite.status === 'pending') {
                            GameInvitesService_1.GameInvitesService.expireInvite(db, invite.id);
                            const s1 = onlineUsers.get(invite.senderId);
                            const s2 = onlineUsers.get(invite.receiverId);
                            if (s1)
                                io.to(s1).emit('game_invite_expired', { inviteId: invite.id });
                            if (s2)
                                io.to(s2).emit('game_invite_expired', { inviteId: invite.id });
        ...
        socket.on('accept_game_invite', async (data) => {
            try {
                if (!userId)
                    return;
                const invite = GameInvitesService_1.GameInvitesService.acceptGameInvite(db, data.inviteId, userId);
                socket.emit('game_invite_accepted', invite);
                const senderSocketId = onlineUsers.get(invite.senderId);
                if (senderSocketId)
                    io.to(senderSocketId).emit('game_invite_accepted', invite);
        ...
        socket.on('decline_game_invite', async (data) => {
            try {
                if (!userId)
                    return;
                GameInvitesService_1.GameInvitesService.declineGameInvite(db, data.inviteId, userId);
                const invite = GameInvitesService_1.GameInvitesService.getInviteById(db, data.inviteId);
                if (invite) {
                    const senderSocketId = onlineUsers.get(invite.senderId);
                    if (senderSocketId)
                        io.to(senderSocketId).emit('game_invite_declined', { inviteId: data.inviteId });
        ...
        socket.on('cancel_game_invite', async (data) => {
            try {
                if (!userId)
                    return;
                // Try to cancel real invite
                const invite = GameInvitesService_1.GameInvitesService.cancelGameInvite(db, data.inviteId, userId);
                socket.emit('game_invite_cancelled', { inviteId: data.inviteId });
                const receiverSocketId = onlineUsers.get(invite.receiverId);
                if (receiverSocketId)
                    io.to(receiverSocketId).emit('game_invite_cancelled', { inviteId: data.inviteId });
        ...
        socket.on('remove_friend', (data) => {
            ...
            try {
                ...
                FriendsService_1.FriendsService.removeFriend(db, userId, data.friendId);
                // 2. Cancel any pending game invites between them
                const cancelledInvites = GameInvitesService_1.GameInvitesService.cancelInvitesBetween(db, userId, data.friendId);
                // 3. Notify both users about cancelled games (Closes Waiting Screen!)
                cancelledInvites.forEach(invite => {
                    const s1 = onlineUsers.get(invite.senderId);
                    const s2 = onlineUsers.get(invite.receiverId);
                    if (s1)
                        io.to(s1).emit('game_invite_cancelled', { inviteId: invite.id });
                    if (s2)
                        io.to(s2).emit('game_invite_cancelled', { inviteId: invite.id });
                    // Update Chat Bubble
                    // (We can use a helper or just direct call)
                    const updatedMsg = MessagesService_1.MessagesService.updateInviteMessage(db, invite.id, 'CANCELLED');
                    if (updatedMsg) {
                        if (s1)
                            io.to(s1).emit('message_updated', updatedMsg);
                        if (s2)
                            io.to(s2).emit('message_updated', updatedMsg);
                    }
                });
        ...
        socket.on('send_friend_request', (data) => {
            ...
            try {
                // Call Service
                const result = FriendsService_1.FriendsService.sendFriendRequest(db, userId, data.friendId);
                // Get Sender Details (To show to Receiver)
                const sender = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(userId);
                // If Auto-Accepted (Both sent requests)
                if (result.status === 'accepted') {
                    const receiverSocketId = onlineUsers.get(data.friendId);
                    const receiver = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(data.friendId);
                    ...
                    if (receiverSocketId)
                        io.to(receiverSocketId).emit('new_friend', { ...sender, status: myStatus });
                }
                // If Pending (Normal Request)
                    ...
                    const friendInfo = db.prepare('SELECT username FROM users WHERE id = ?').get(data.friendId);
        ...
        socket.on('accept_friend_request', (data) => {
            if (!userId)
                return;
            try {
                FriendsService_1.FriendsService.acceptFriendRequest(db, data.friendshipId);
                // Fetch details
                const me = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(userId);
                const them = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(data.friendId);
                // 🛠️ FIX: Inject REAL status
                const myStatus = onlineUsers.has(userId) ? 'online' : 'offline';
                const theirStatus = onlineUsers.has(data.friendId) ? 'online' : 'offline';
                ...
                if (themSocket)
                    io.to(themSocket).emit('new_friend', { ...me, status: myStatus });
            }
            catch (error) {
                socket.emit('error', { message: error.message });
            }
        ...
        socket.on('decline_friend_request', (data) => {
            if (!userId)
                return;
            try {
                const result = FriendsService_1.FriendsService.declineFriendRequest(db, data.friendshipId);
        ...
        socket.on('block_user', (data) => {
            if (!userId)
                return;
            try {
                FriendsService_1.FriendsService.blockUser(db, userId, data.blockedUserId);
                // Fetch blocked user details for my list
                const blockedUser = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(data.blockedUserId);
        ...
        socket.on('unblock_user', (data) => {
            if (!userId)
                return;
            try {
                FriendsService_1.FriendsService.unblockUser(db, userId, data.blockedUserId);
                ...
                if (friendship) {
                    const friend = db.prepare('SELECT id, username, Avatar as avatarUrl FROM users WHERE id = ?').get(data.blockedUserId);
        ...
        socket.on('client:join', async (data) => {
            try {
                ...
                if (!gameUserId || !username) {
                    socket.emit('server:error', { message: 'User must be authenticated to join' });
                    return;
                }
                console.log(`🎮 ${username} (ID: ${gameUserId}) joining matchmaking`);
                // Try to find a match
                const pairing = matchmakingService.enqueue(socket, gameUserId, username);
                if (!pairing) {
                ...
                const room = gameRoomManager.createRoom({ socket: pairing.player1.socket, userId: pairing.player1.userId, username: pairing.player1.username }, { socket: pairing.player2.socket, userId: pairing.player2.userId, username: pairing.player2.username });
                // Notify both players they're matched
                pairing.player1.socket.emit('server:queue', {
                    status: 'matched',
                    roomId: room.id
                });
                pairing.player2.socket.emit('server:queue', {
                    status: 'matched',
                    roomId: room.id
                });
            }
            catch (error) {
                console.error('Error in client:join:', error);
        ...
        socket.on('client:ready', (data) => {
            try {
                ...
                if (room) {
                    room.setPlayerReady(gameUserId);
                    console.log(`✅ ${gameUserId} ready in room ${room.id}`);
                }
            }
            catch (error) {
                console.error('Error in client:ready:', error);
                socket.emit('server:error', { message: error.message });
            }
        });
        socket.on('client:input', (data) => {
            try {
                ...
                const room = gameRoomManager.getRoomByUserId(gameUserId);
                if (room) {
                    room.handleInput(gameUserId, data.command);
                }
            }
            catch (error) {
                console.error('Error in client:input:', error);
            }
        });
        // ==================== END GAME HANDLERS ====================
        socket.on('disconnect', () => {
            if (userId) {
                ...
                const cancelledInvites = GameInvitesService_1.GameInvitesService.cancelInvitesFromSender(db, userId);
                cancelledInvites.forEach(invite => {
                ...
                const room = gameRoomManager.getRoomByUserId(userId);
                if (room) {
                    // Notify opponent
                    for (const player of room.getPlayers().values()) {
                        if (player.userId !== userId) {
                            player.socket.emit('server:opponent-left');
                        }
                    }
...
function notifyFriendsStatus(db, io, userId, status) {
    try {
        const friends = FriendsService_1.FriendsService.getFriendsToNotify(db, userId);
        friends.forEach((friend) => {
            // if (FriendsService.isBlocked(userId, friend.id)) return;
            const friendSocketId = onlineUsers.get(friend.id);
            if (friendSocketId)
                io.to(friendSocketId).emit('friend_status_change', { userId, status });
        });
    }
    catch (error) { }
}
function sendOnlineFriendsList(db, io, socket, userId) {
    try {
        const friends = FriendsService_1.FriendsService.getFriends(db, userId);
        const blocked = FriendsService_1.FriendsService.getBlockedUsers(db, userId);
        const allContacts = [...friends, ...blocked];
        const onlineFriends = allContacts.filter((allContacts) => {
