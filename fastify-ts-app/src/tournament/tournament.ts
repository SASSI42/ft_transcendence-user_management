...
export class Tournament {
	...
	public status: TournamentStatus = "registering";
	...
	public register(alias: string, socket: Socket): "joined" | "reconnected" {
		...
		if (existing) {
			...
			if (activeMatch) {
				// TODO: Once user management is integrated, use actual userId from auth token
				const userId = getOrCreateUserId(canonical);
				const result = this.roomManager.rejoinRoom(activeMatch.roomId, socket, userId, canonical);
				switch (result) {
					case "approved":
	...
	public claimReadyMatch(): { matchId: string; aliases: [string, string] } | null {
		...
		for (const match of this.bracket.getPendingMatches()) {
			...
			try {
				...
				console.error(`[tournament] failed to mark match active`, {
					code: this.code,
					matchId: match.id,
					error,
				});
	...
	public markMatchFinished(matchId: string, aliases: [string, string], winnerAlias?: string | null): void {
		...
		if (winnerAlias) {
			try {
				...
				console.error(`[tournament] failed to record result`, {
					code: this.code,
					matchId,
					error,
				});
	...
	private persistState(): void {
		try {
			...
			console.error("[tournament] failed to persist state", { code: this.code, error });
		}
	}
	...
	public destroy(): void {
		...
		try {
			...
			console.error("[tournament] failed to delete", { code: this.code, error });
		}
	}
