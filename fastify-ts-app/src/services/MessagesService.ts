...
export class MessagesService {
    ...
    static getMessageHistory(db: Database, userId1: number, userId2: number, limit: number = 50, offset: number = 0) {
    ...
    static markAsRead(db: Database, messageId: string, userId: number) {
    ...
    static getUnreadCount(db: Database, userId: number) {
        const res = db.prepare(`SELECT COUNT(*) as count FROM messages WHERE receiver_id = ? AND read = 0 AND is_blocked = 0`).get(userId) as { count: number };
        return res.count;
    }

    static getUnreadMessagesBySender(db: Database, userId: number) {
        const stmt = db.prepare(`
