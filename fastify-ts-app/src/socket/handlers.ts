...
export function setupSocketHandlers(db: Database, io: Server) {
    io.on('connection', (rawSocket: Socket) => {
        ...
        socket.on('client:join', async (data: { username: string; userId?: number }) => {
            try {
                ...
                const room = gameRoomManager.createRoom(
                    { socket: pairing.player1.socket, userId: pairing.player1.userId, username: pairing.player1.username },
                    { socket: pairing.player2.socket, userId: pairing.player2.userId, username: pairing.player2.username }
                );
        ...
        socket.on('client:ready', (data: { ready: boolean }) => {
            try {
                const gameUserId = socket.data.user?.id;
        ...
        socket.on('client:input', (data: { command: InputCommand }) => {
            try {
                const gameUserId = socket.data.user?.id;
                if (!gameUserId) return;
